...
xu li du lieu trong cac file ctu: 1, 3, 4, 5 --> training set:
    input  {srcPort, state,sTOS, srcPort, bytePerPkt ,avgPkt, avgByte}
...
def testDataPrepocess():
    ...
    dataPrepocess('data_raw/cap2.binetflow', testFile, testLabel)
    dataPrepocess('data_raw/cap4.binetflow', testFile, testLabel)
    dataPrepocess('data_raw/cap5.binetflow', testFile, testLabel)
    testFile.close()
def trainDataPrepocess():
    ...
    dataPrepocess('data_raw/cap9.binetflow', trainFile, trainLabel)
    dataPrepocess('data_raw/cap13.binetflow', trainFile, trainLabel)
    trainFile.close()
    trainLabel.close()
