...
#input  = netfolw
#output  data = [sIp, sPort, dIp, dPort, state, sTos, bytePerPkt, avgPkt, avgByte]
#and label = [0 or 1]
def flowToArray(flow):
    ...
    if (dur != 0):
        try:
            ...
            if (listFlow[2] == 'icmp'):
                data = [sIp, 0, dIp, 0, state, sTos, bytePerPkt, avgPkt, avgByte]
            # goi tin icmp khong co port => gan dstport = 1 ang srcport = 1
            elif (listFlow[2] == 'arp'):
                data = [sIp, 1, dIp, 1, state, sTos, bytePerPkt, avgPkt, avgByte]
            #doi tin esp
            elif (listFlow[2] == 'esp'):
                data = [sIp, 2, dIp, 2, state, sTos, bytePerPkt, avgPkt, avgByte]
            #goi tin tcp khong co dstport
            elif (listFlow[2] == 'tcp' and listFlow[7] == ''):
                data = [sIp, int(listFlow[4].strip()), dIp, 3, state, sTos, bytePerPkt, avgPkt, avgByte]
            #goi tin igmp
            elif (listFlow[2] == 'igmp'):
                data = [sIp, 4, dIp, 4, state, sTos, bytePerPkt, avgPkt, avgByte]
            else:
                data = [sIp, int(listFlow[4].strip()), dIp, int(listFlow[7]), state, sTos, bytePerPkt, avgPkt, avgByte]
